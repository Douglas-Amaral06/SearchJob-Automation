...
from app.config import JOBICY_CACHE_TTL_SECONDS
...
async def _obter_feed() -> list[dict[str, Any]]:
    ...
    if _cache and agora - _cache[1] < JOBICY_CACHE_TTL_SECONDS:
        return _cache[0]
    ...
    async with _lock():
        ...
        try:
            ...
            logger.warning("Jobicy indisponível: %s", type(erro).__name__)
...
async def buscar_vagas_jobicy(
    ...
    if modalidade != "Remoto":
        return {"fonte": "Jobicy", "total_fonte": 0, "vagas": []}

    vagas: list[dict] = []
    for job in await _obter_feed():
        titulo = str(job.get("jobTitle") or "").strip()
        url = str(job.get("url") or "").strip()
        descricao = remover_html(job.get("jobDescription"))
        local = str(job.get("jobGeo") or "").strip() or "Remoto"
        ...
        data = normalizar_data(job.get("pubDate"))
        ...
        vagas.append(
            {
                "id_externo": f"jobicy:{id_externo}",
                "titulo": titulo,
                "empresa": str(job.get("companyName") or "Empresa não informada").strip(),
                "local": local,
                "modalidade": "Remoto",
                "url_candidatura": url,
